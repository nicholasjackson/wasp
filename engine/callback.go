...
func (w *Wasm) AddCallback(name string, f interface{}) error {
	...
	w.callbackFunctions[name] = wasmer.NewFunction(
		w.store,
...
func (w *Wasm) addCallbacks(namespace string, io *wasmer.ImportObject) {
	io.Register(namespace, w.callbackFunctions)
}
